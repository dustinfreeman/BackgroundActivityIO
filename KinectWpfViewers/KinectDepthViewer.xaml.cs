...
namespace Microsoft.Samples.Kinect.WpfViewers
{
    ...
    public partial class KinectDepthViewer : ImageViewer
    {
        ...
        // these end up down-shifting the depth pixel intensity by 1 or 2.
        ...
        protected override void OnKinectChanged(FakeKinectSensor oldKinectSensor, FakeKinectSensor newKinectSensor)
        {
            if (oldKinectSensor != null)
            {
                oldKinectSensor.FDepthFrameReady -= this.FDepthFrameReady;
                kinectDepthImage.Source = null;
            }
            ResetFrameRateCounters();
            ...
            {
                newKinectSensor.FDepthFrameReady += new FDepthFrameReadyEvent(FDepthFrameReady);
            }
        }
        ...
        private void FDepthFrameReady(FDepthFrame depthFrame)
        {
            this.pixelData = depthFrame.depthFrameShort;
            this.depthFrame32 = new byte[depthFrame.Width * depthFrame.Height * Bgr32BytesPerPixel];

            byte[] convertedDepthBits = this.ConvertDepthFrame(this.pixelData, depthFrame);

            // A WriteableBitmap is a WPF construct that enables resetting the Bits of the image.
            // This is more efficient than creating a new Bitmap every frame.
            if (outputBitmap == null ||
                outputBitmap.Width != depthFrame.Width ||
                outputBitmap.Height != depthFrame.Height)
            {
                this.outputBitmap = new WriteableBitmap(
                    depthFrame.Width,
                    depthFrame.Height,
        ...
        private byte[] ConvertDepthFrame(short[] depthFrame, FDepthFrame fdepthFrame)
        {
            int tooNearDepth, tooFarDepth, unknownDepth;
            tooNearDepth = 0; tooFarDepth = 4095; unknownDepth = -1;

            return _ConvertDepthFrame(depthFrame, tooNearDepth, tooFarDepth, unknownDepth);
        }
        ...
        private byte[] _ConvertDepthFrame(short[] depthFrame, int tooNearDepth, int tooFarDepth, int unknownDepth)
        {
